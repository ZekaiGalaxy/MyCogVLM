...
do
  dir_num=$(printf "%04d" $i)
  TORCH_CUDA_ARCH_LIST="7.0" torchrun --standalone --nnodes=1 --nproc-per-node=4 inference.py --from_pretrained cogvlm-chat --version chat --english --fp16 --input_file=/f_ndata/zekai/test_json/$dir_num.jsonl --output_file=/f_ndata/zekai/caption_data/$dir_num.txt
done
